...
for ticker_symbol in top_ten_tickers:
    ...
    if not ticker_history.empty:
        ...
        if not stock_data['Close'].empty:
            ...
            sharpe_ratios[ticker_symbol] = sharpe_ratio  
        else:
            print(f"Close data is empty for {ticker_symbol}. Skipping...")
    else:
        print(f"No historical data available for {ticker_symbol}. Skipping...")

sorted_tickers = sorted(sharpe_ratios.items(), key=lambda x: x[1], reverse=True)

print("Top ten tickers based on Sharpe ratio:")
for ticker, ratio in sorted_tickers[:10]:
    action = 'Buy' if ratio > sharpe_threshold else 'Sell'
